...
int32_t I2cReadDataWait(I2C_Data *data, const TickType_t delay, const TickType_t xMaxBlockTime){
    ...
    //---2. Initiate sending data (to write register address)
    error = I2cWriteData(data);
    if (ERROR_NONE != error){
        goto exitError0;
    }

    //---3. Wait for binary semaphore to tell us that the write is done!
    if( xSemaphoreTake( semHandle, xMaxBlockTime ) != pdTRUE ){
        // The call to xSemaphoreTake() timed out.
        error = ERR_TIMEOUT;
        goto exitError0;
    ...
    // Check for transmission error after write
    if(I2cGetTaskErrorStatus()){
        I2cSetTaskErrorStatus(false);
        error = ERROR_I2C_HANG_RESET;
        goto exitError0;
    ...
    //---4. Delay if needed (if the device needs time to prepare the data)
    if(delay > 0) {
        vTaskDelay(delay);
    }
